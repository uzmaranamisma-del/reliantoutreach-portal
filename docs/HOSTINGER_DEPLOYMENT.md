...
After future pushes to `main`, Hostinger automatically rebuilds and redeploys the application. Pending committed database migrations run at the start of the production build when `DATABASE_URL` is configured; never mutate the production schema manually.
